...
function mediaMarkup(macro, compact = false) {
  ...
  const video = hoverMedia?.type === 'video' && hoverMedia.sourceUrl ? `<iframe class="macro-cover-hover-video" data-preview-src="https://www.youtube.com/embed/${new URL(hoverMedia.sourceUrl).searchParams.get('v')}?autoplay=1&mute=1&loop=1&playlist=${new URL(hoverMedia.sourceUrl).searchParams.get('v')}&controls=0&playsinline=1" title="${macro.name} working demo" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen loading="lazy"></iframe>` : hoverMedia?.type === 'video' ? `<video class="macro-cover-hover-video" src="${hoverMedia.src}" muted loop playsinline preload="none"></video>` : hoverMedia?.type === 'gif' ? `<img class="macro-cover-hover-video" src="${hoverMedia.src}" alt="${macro.name} working demo" loading="lazy" />` : macro.coverVideo ? `<video class="macro-cover-hover-video" src="${macro.coverVideo}" muted loop playsinline preload="none"></video>` : '';
...
export function demoMediaMarkup(macro) {
  ...
  const media = videoId ? `<iframe src="https://www.youtube.com/embed/${videoId}?autoplay=0&mute=1&loop=1&playlist=${videoId}&playsinline=1" title="${macro.name} working demo" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen loading="lazy"></iframe>` : demo.type === 'video' ? `<video src="${demo.src}" muted loop playsinline controls preload="metadata"></video>` : `<img src="${demo.src}" alt="${demo.alt || `${macro.name} working demo`}" loading="lazy" />`;
