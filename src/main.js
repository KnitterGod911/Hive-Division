...
function shell(content) {
  ...
  try {
    ...
    app.innerHTML = `<div style="padding: 40px; color: #ff6b6b; background: #0a0b0d; font-family: monospace; font-size: 14px;">
      <h2 style="color: #ff9999; margin-top: 0;">Error: ${error.name}</h2>
      <p><strong>${error.message}</strong></p>
      <pre style="overflow: auto; background: #111; padding: 10px; border-radius: 4px;">${error.stack}</pre>
    </div>`;
  }
}
...
function beeDetailPage(beeData) {
  const beeTheme = beeData.theme || { base: '#b7bab2', soft: 'rgba(183, 186, 178, 0.22)', accent: '#f2f5ec' };
  ...
  const abilityText = beeData.ability || beeData.description;
  const obtainmentSection = beeData.obtainment ? `<section class="bee-info-panel"><span class="detail-label">How to obtain</span><h2>Unlocking</h2><p>${beeData.obtainment}</p></section>` : '';
  const giftedText = beeData.giftedAbility || 'Gifted ability information is maintained on the BSS Wiki.';
  const colorName = beeData.color === 'colorless' ? 'Colorless' : beeData.color.charAt(0).toUpperCase() + beeData.color.slice(1);
  const effectClass = beeData.backgroundEffect || 'generic';
  const ambientMarkup = {
    generic: '<span class="scene-dot scene-dot-a"></span><span class="scene-dot scene-dot-b"></span><span class="scene-dot scene-dot-c"></span>',
    photon: '<span class="beam beam-1"></span><span class="beam beam-2"></span><span class="beam beam-3"></span><span class="beam beam-4"></span>',
    precise: '<span class="hitmark h-1"></span><span class="hitmark h-2"></span><span class="hitmark h-3"></span><span class="hitmark h-4"></span>',
    vicious: '<span class="stinger s-1"></span><span class="stinger s-2"></span><span class="stinger s-3"></span><span class="stinger s-4"></span>',
    fire: '<span class="flame f-1"></span><span class="flame f-2"></span><span class="flame f-3"></span><span class="flame f-4"></span>',
    bubble: '<span class="bubble b-1"></span><span class="bubble b-2"></span><span class="bubble b-3"></span><span class="bubble b-4"></span>',
    windy: '<span class="wind w-1"></span><span class="wind w-2"></span><span class="wind w-3"></span><span class="wind w-4"></span>',
    gummy: '<span class="goo g-1"></span><span class="goo g-2"></span><span class="goo g-3"></span><span class="goo g-4"></span>',
    bear: '<span class="paw paw-1"></span><span class="paw paw-2"></span><span class="paw paw-3"></span>',
    digital: '<span class="code c-1"></span><span class="code c-2"></span><span class="code c-3"></span>',
    vector: '<span class="vector v-1"></span><span class="vector v-2"></span><span class="vector v-3"></span>',
    tabby: '<span class="paw paw-1"></span><span class="paw paw-2"></span><span class="scratch scratch-1"></span>',
    blue: '<span class="pulse pulse-1"></span><span class="pulse pulse-2"></span><span class="pulse pulse-3"></span>',
    red: '<span class="pulse pulse-1"></span><span class="pulse pulse-2"></span><span class="pulse pulse-3"></span>',
    bomb: '<span class="bomb b-1"></span><span class="bomb b-2"></span><span class="bomb b-3"></span>',
    fuzzy: '<span class="fuzz fuzz-1"></span><span class="fuzz fuzz-2"></span><span class="fuzz fuzz-3"></span>'
  }[effectClass] || '<span class="scene-dot scene-dot-a"></span><span class="scene-dot scene-dot-b"></span><span class="scene-dot scene-dot-c"></span>';

  return `<section class="bee-detail-hero bee-themed page-wrap" style="--bee-theme: ${beeTheme.base}; --bee-theme-soft: ${beeTheme.soft}; --bee-theme-accent: ${beeTheme.accent};"><div class="bee-detail-ambient bee-ambient-${effectClass}" aria-hidden="true">${ambientMarkup}</div><a class="back-link" href="/bees/${beeData.rarity}">${icon('arrow-up-right', 15)} Back to ${beeData.rarity} bees</a><div class="bee-detail-grid"><div class="bee-detail-image bee-card-${beeData.color}"><img src="${beeData.image}" alt="${beeData.name}" /></div><div class="bee-detail-identity"><span class="eyebrow">Bee directory / ${beeData.rarity}</span><h1>${beeData.name}</h1><p>${abilityText}</p><a class="wiki-source" href="${beeData.wikiUrl}" target="_blank" rel="noreferrer">Open BSS Wiki source ${icon('external-link', 14)}</a></div></div></section><section class="bee-detail-content bee-themed page-wrap section-space" style="--bee-theme: ${beeTheme.base}; --bee-theme-soft: ${beeTheme.soft}; --bee-theme-accent: ${beeTheme.accent};"><div class="bee-info-grid"><section class="bee-info-panel"><span class="detail-label">Bee profile</span><h2>Overview</h2><p><strong>Rarity:</strong> ${beeData.rarity}<br /><strong>Color:</strong> ${colorName}<br /><strong>Role:</strong> ${abilityText}</p></section><section class="bee-info-panel"><span class="detail-label">Ability</span><h2>What it does</h2><p>${abilityText}</p></section>${obtainmentSection}<section class="bee-info-panel"><span class="detail-label">Ability tokens</span><div class="token-list">${beeData.tokens.map((token) => `<div>${icon('hexagon', 14)}<span>${token}</span></div>`).join('')}</div></section><section class="bee-info-panel"><span class="detail-label">Gifted ability</span><h2>${giftedText}</h2><p>This hive-wide bonus is granted when this bee becomes gifted. The BSS Wiki data is shown here as the bee's gifted stat effect.</p></section>${giftedSpecial}</div></section>`;
}
