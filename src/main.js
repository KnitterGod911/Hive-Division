...
function beeDetailPage(beeData) {
  ...
  const obtainmentText = beeData.obtainment || 'This bee is obtained through normal progression, and the exact unlock route is listed on the Bee Swarm Simulator Wiki.';
  const giftedText = beeData.giftedAbility || 'The gifted version of this bee gains a stronger hive-wide bonus.';
  return `<section class="bee-detail-hero page-wrap"><a class="back-link" href="/bees/${beeData.rarity}">${icon('arrow-up-right', 15)} Back to ${beeData.rarity} bees</a><div class="bee-detail-grid"><div class="bee-detail-image bee-card-${beeData.color}"><img src="${beeData.image}" alt="${beeData.name}" /></div><div class="bee-detail-identity"><span class="eyebrow">Bee directory / ${beeData.rarity}</span><h1>${beeData.name}</h1><p>${beeData.description}</p><a class="wiki-source" href="${beeData.wikiUrl}" target="_blank" rel="noreferrer">Open BSS Wiki source ${icon('external-link', 14)}</a></div></div></section><section class="bee-detail-content page-wrap section-space"><div class="bee-info-grid"><section class="bee-info-panel"><span class="detail-label">About this bee</span><h2>How it helps</h2><p>${beeData.description}</p></section><section class="bee-info-panel"><span class="detail-label">Ability</span><h2>What it does</h2><p>${abilityText}</p></section><section class="bee-info-panel"><span class="detail-label">How to obtain</span><h2>Unlocking</h2><p>${obtainmentText}</p></section><section class="bee-info-panel"><span class="detail-label">Ability tokens</span><div class="token-list">${beeData.tokens.map((token) => `<div>${icon('hexagon', 14)}<span>${token}</span></div>`).join('')}</div></section><section class="bee-info-panel"><span class="detail-label">Gifted ability</span><h2>${giftedText}</h2><p>This hive-wide bonus is granted when this bee becomes gifted. Details are sourced from the BSS Wiki.</p></section>${giftedSpecial}</div></section>`;
}
