...
function beeDetailPage(beeData) {
  return `<section class="bee-detail-hero page-wrap"><a class="back-link" href="/bees/${beeData.rarity}">${icon('arrow-up-right', 15)} Back to ${beeData.rarity} bees</a><div class="bee-detail-grid"><div class="bee-detail-image bee-card-${beeData.color}"><img src="${beeData.image}" alt="${beeData.name}" /></div><div class="bee-detail-identity"><span class="eyebrow">Bee directory / ${beeData.rarity}</span><h1>${beeData.name}</h1><p>${beeData.description}</p><a class="wiki-source" href="${beeData.wikiUrl}" target="_blank" rel="noreferrer">Open BSS Wiki source ${icon('external-link', 14)}</a></div></div></section><section class="bee-detail-content page-wrap section-space"><div class="bee-info-grid"><section class="bee-info-panel"><span class="detail-label">About this bee</span><h2>What it does</h2><p>${beeData.description}</p></section><section class="bee-info-panel"><span class="detail-label">Ability</span><h2>How it works</h2><p>${beeData.ability}</p></section><section class="bee-info-panel"><span class="detail-label">Ability tokens</span><div class="token-list">${beeData.tokens.map((token) => `<div>${icon('hexagon', 14)}<span>${token}</span></div>`).join('')}</div></section><section class="bee-info-panel"><span class="detail-label">Gifted bonus</span><h2>${beeData.giftedBonus}</h2><p>The gifted bonus applies when this bee is gifted. Event bees do not have favorite treats.</p></section></div></section>`;
}
